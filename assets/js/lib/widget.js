    ...
    **/
    prototype : {
        ...
        **/
        ...
        /**
        @todo This method should not be part of everyWidget, could be a utility class or
        a module included only in the places that will use it
        **/
        positionOverlay : function positionOverlay(whichElement, relativeToElement) {
            var dropdown = this;
            this.position = relativeToElement.findPos();
            ...
            var overlayData = {
                w : whichElement.width(),
                h : whichElement.height()
            };

            var relativePosition = relativeToElement.findPos();

            var canBeLeft = function(relativePosition) {
                var x = relativePosition.x - overlayData.w;
                if ( (x) > 0 ) {
            ...
            var canBeTop = function(relativePosition) {
                var y = relativePosition.y - overlayData.h - window.scrollY;
                return y > 0;

            };
            
            if (relativeToElement.length > 0) {
                var elementData      =  { 
                    position : relativePosition,
                    w        : relativeToElement.width(),
                    h        : relativeToElement.height()
                };
                
            }
                        
            if (canBeTop(relativePosition)) {
                whichElement.css({
                    top : dropdown.position.y - overlayData.h
                });
            } else {
                whichElement.css({
                    top : dropdown.position.y + relativeToElement.height()
                });
            }
            
            if (canBeLeft(relativePosition)) {
                whichElement.css({
                ...
                var overlayRightPosition = dropdown.position.x + elementData.w + (overlayData.w * 2);
                var left = dropdown.position.x + elementData.w;
                
                // rest the overlay width if it doesnt fit left and right
                if (overlayRightPosition > documentData.w) {
                    left = left - overlayData.w;
                }
